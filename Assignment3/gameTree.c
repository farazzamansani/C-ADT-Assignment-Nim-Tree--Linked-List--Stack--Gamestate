/*
 *	GameTree ADT
 *	author Faraz Zamansani ID: 386070 and Bradley Hayes ID:222671  :P
 *	version 10th October 2015
 *	
...
int getLevel(gameTree t)
{
	...
	if (isEmptyGT(t))
	{
		fprintf(stderr, "getLevel: empty game tree");
		exit(1);
	}
...
{		
	...
	gameTree tree;
	init_gameTree(&tree, true, NULL, NULL);
	tree->root = getTNChild(t->root);
...
void setData(gameTree t,void *o)
{
	...
	if (t==NULL)
	{
		fprintf(stderr, "setData: NULL game tree");
		exit(1);
	}
	setTNData(t->root, o);
...
void setLevel(gameTree t,int l)
{
	...
	if (isEmptyGT(t))
	{
		fprintf(stderr, "setLevel: empty game tree");
		exit(1);
	}	
...
void setChild(gameTree t,gameTree c)
{
	...
	if (isEmptyGT(t))
	{
		fprintf(stderr, "setChild: empty game tree");
		exit(1);
	}
...
void setSibling(gameTree t,gameTree s)
{
	...
	if (isEmptyGT(t))
	{
		fprintf(stderr, "setSibling: empty game tree");
		exit(1);
	}
...
void generateLevelDF(gameTree t,stack k)
{
	...
	//[ LOCAL VARIABLES ]
	tNode tn;										//used to refer to tnode just created
	tNode sib;										//when sibling is created for last tn created.
	int level = (getLevel(t)) + 1;					//level to be used for new child and its siblings
	gameState new_gs;								//used for referencing new gamestates in our new tnodes.
	gameTree new_gt;								//used for temporary gametree reference for creating and adding to the stack
	gameTree sib_gt;								//used for creating next sibling
	int count = getCount((gameState)getData(t));	//deincrements for each new gamestate initialised.
	int worth = 0;									//default worth value for initialising gamestates
	bool computer;									//will the computer be taking the next go?

	//who are these new move choices for..
	if (level%2==0)
	{
		computer = true;//human will choose next (BUT we evaluate from oponents perspective so computer is still true)
	}
	else
	{
		computer = false;//computer will choose next
	}

	//First the Child node is specifically handled and becomes preconditions for the loop.
	count--;											//first deincrement. 1st choice of remove 1 match.
	init_gameState(&new_gs, count, worth);				//games state to be used in 1st tNode (child)
	setWorth(new_gs, evaluateState(new_gs,computer));	//set worthyness of this move choice
	init_TNode(&tn, &new_gs, level );					//a child node is born :)
	init_gameTree(&new_gt, false, new_gs, level);		//wrap the newborn in a gametree blanket :)
	setChild(t, new_gt);								//connect the parent to the child.
	push(k, new_gt);									//push to stack.
	
	//number of siblings will be generated untill MAX_MOVES is reached or no matches will remain
	int i = 1;						//loop counter to make sure no more than MAX_MOVES is created
	while (count>1 && i<MAX_MOVES)
	{
		//do the same for siblings as done for child.
		i++;
		count--;
		init_gameState(&new_gs, count, worth);
		setWorth(new_gs, evaluateState(new_gs, computer));
		init_TNode(&sib, &new_gs, level);
		init_gameTree(&sib_gt, false, new_gs, level);
		setSibling(new_gt, sib_gt);	//connect child or previouse sibling to this new sibling
		push(k, sib_gt);
		new_gt = sib_gt; //move sibling along for next loop execution.
	}
...
/*
	...
	*					stack.
	...
	Finally, the next tree to be considered
...
void buildGameDF(gameTree t, stack k, int d)
{
	...
	//[ LOCAL VARIABLES ]
	gameTree next_gt;	//stores the top of stack(next to be processed)
	...
	{
		next_gt = t; //to start with using initial gametree

		push(k, next_gt); //push starting point of loop onto the stack

		while (!isEmptyS(k))
		{
			//is there a child
			if (isEmptyGT(getChild(next_gt)))
			{
				//no child build child + siblings and push em on to stack
				if (getLevel(next_gt)<d)
				{
					generateLevelDF(next_gt, k);
				}
				
			}
			else
			{
				// yes then push that child (inside a gametree)
				push(k, getChild(next_gt)); //getchild makes a GT

				//then do the same for each sibling
				next_gt = getChild(next_gt);
				while (getTNSibling(next_gt->root) !=NULL)
				{
					//init_gameTree(&tempt_gt, true, NULL, NULL);
					//setData(tempt_gt, tn);
					push(k, getSibling(next_gt));

					next_gt = getSibling(next_gt);//move along
				}
			}
			next_gt = top(k); //change to next game tree to process
			pop(k);//and remove it from stack 
		}
